...
function determineIncremental()
{
	...
	//incrElem = currentSlide.find(".incremental  .item")
	incrElem = currentSlide.find(".increment")
...
function nextStep()
{
	...
	if (incrCurr >= incrSteps) {
		...
		if (incrCode && elem.hasClass('command')) {
			incrElem.eq(incrCurr).fadeIn().jTypeWriter({duration:1.0})
		} else {
      incrElem.eq(incrCurr).fadeIn();
		}
		incrCurr++
	}
}
